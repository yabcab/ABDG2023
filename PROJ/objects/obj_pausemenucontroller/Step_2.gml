if obj_LGplayer.pause
{
	...
	if disp_state = 0
	{
		...
		if selector_targetheight = 350
		{
			if obj_LGplayer.key_jump_press
			{
				...
				pause_state = obj_LGplayer.previous_pause
				obj_LGplayer.pause = 0
	...
	if disp_state = 1
	{
		...
		if obj_LGplayer.key_attack_press
		{
		...
		if selector_targetheight = 200
		{
			...
			if obj_LGplayer.key_left
				if sfx_vol > 0
					sfx_vol -= 0.01
				else
					sfx_vol = 0
			if obj_LGplayer.key_right
				if sfx_vol < 1
					sfx_vol += 0.01
				else
					sfx_vol = 1
			ini_write_real("settings","sfx_vol",sfx_vol)
			audio_group_set_gain(audio_sound_lg,sfx_vol,0)
		}
