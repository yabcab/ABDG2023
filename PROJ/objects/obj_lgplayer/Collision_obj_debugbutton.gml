if other.solid
{
	...
	// dash
	if instance_place(x + hspeed,y + 1,obj_LGsolid) && !instance_place(x + hspeed,y - 12,obj_LGsolid)
	{
		dash_charge = 1
		pounding = 0
	}
	if !instance_place(x,y-1,obj_LGsolid)
		dashing = 0
	
	if state = playerstate.hurt && (instance_place(x,y+1,obj_LGsolid) || instance_place(x+1,y,obj_LGsolid) || instance_place(x-1,y,obj_LGsolid)) && hurttime > 10
	{
		state = idlestate
		hurttime = 0
	}
	
	if state = playerstate.balloon_dash && instance_place(x,y+1,obj_LGsolid)
		state = idlestate
}	
